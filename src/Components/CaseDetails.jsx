...
const CaseDetails = ({ id }) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${import.meta.env.VITE_BASE_URL}/getCaseDetailsByCaseId/${id}`)
      .then((res) => {
  ...
  function formatNumberToDay(num) {
    if (num == 3) return "3rd day";
    else return num + "th day";
  }
              ...
              <div className="flex flex-col items-center bg-white p-6 shadow-2xl border-2 shadow-black/20 rounded-2xl max-md:mx-4 max-md:p-2">
                ...
                <table className="border-separate border-spacing-x-5 border-spacing-y-2">
                  ...
                  <tr>
                    ...
                    <td
                      className={`${
                        victim?.vaccination_status === "Vaccinated"
                          ? "bg-green-400"
                          : "bg-red-400"
                      } text-center rounded-lg`}
                    >
                  ...
                  <tr>
                    ...
                    <td
                      className={`text-center px-2 rounded-md ${
                        victim?.wound_severity === "Mild" && "bg-green-400"
                      } ${
                        victim?.wound_severity === "Moderate" && "bg-orange-400"
                      } ${victim?.wound_severity === "Severe" && "bg-red-400"}`}
                    >
                    ...
                    <div className="bg-slate-100 rounded-lg p-3">
                      {doseDetails?.map((ele) => (
                        <h2 className="font-semibold">
                          {formatNumberToDay(ele.dose)} : {ele.dose_date}
                        </h2>
                      ))}
                    </div>
                  </tr>
